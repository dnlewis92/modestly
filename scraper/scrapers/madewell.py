"""
Madewell scraper — Women's Skirts.
Madewell (J.Crew Group) uses a React storefront.
"""
...
class MadewellScraper(BaseScraper):
    ...
    SKIRTS_URL = "https://www.madewell.com/womens-clothing/skirts"
    ...
    async def _scrape_products(self, page: Page) -> list[RawProduct]:
        ...
        try:
            ...
            cards = await page.query_selector_all("[class*='ProductCard'], [class*='product-card'], li[class*='product']")
            print(f"[Madewell] Found {len(cards)} cards")

            for card in cards[:60]:
                try:
                    name_el = await card.query_selector("[class*='product-name'], [class*='ProductName'], p[class*='name'], h2, h3")
                    name = await name_el.inner_text() if name_el else None

                    link_el = await card.query_selector("a")
                    href = await link_el.get_attribute("href") if link_el else None
                    if href and not href.startswith("http"):
                        href = "https://www.madewell.com" + href
            ...
            print(f"[Madewell] Scrape error: {e}")
        return products
