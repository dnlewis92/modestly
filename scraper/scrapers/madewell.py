"""
Madewell scraper — uses their internal search JSON API.
"""
...
API_URL = (
    "https://www.madewell.com/search?q=skirt&prefn1=division&prefv1=Womens"
    "&start={start}&sz=48&format=json"
)
...
class MadewellScraper(BaseScraper):
    ...
    async def scrape(self) -> list[RawProduct]:
            ...
            async with httpx.AsyncClient(headers=HEADERS, timeout=30, follow_redirects=True) as client:
                for start in [0, 48]:
                    ...
                    if r.status_code != 200:
                        print(f"[Madewell] API returned {r.status_code}")
                        break
                    data = r.json()
                    ...
                    for item in hits:
                        try:
                            ...
                            if url and not url.startswith("http"):
                                url = "https://www.madewell.com" + url
        ...
        print(f"[Madewell] Found {len(products)} raw products")
        for p in products:
